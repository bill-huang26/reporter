...
* [How can I get reporter to break/wrap long words?](#wordbreak)
* [Is there a way to insert my own custom RTF codes?](#codes)
* [How come my reports are messed up on Mac?](#mac)
...
The RTF, PDF, HTML, and DOCX output types support Courier, Arial, Times New Roman, 
and SimSun.  SimSun is a font specifically for Chinese characters.

Note that the "Courier" font keyword actually produces "Courier New" on the output
report.  
...
You may also add styling via code insertion.
See the vignette 
[Example 17: Code Insertion](reporter-codeinsert.html) for an illustration.
...
Note that, by default, **reporter** will still break 
the page if the number of rows on a page exceeds the calculated line count. If
you want to disable the automatic paging, and have 100% control
over when page breaks occur, turn automatic page breaking off using 
`report_option(auto_page = FALSE)`. 

You can also control the line count on a page using the `line_count`
parameter on the `options_fixed()` or `report_options` functions. 

For a full explanation of the page breaking options, See the vignette 
[Example 18: Report Options](reporter-report_options.html).
...
function to identify a variable to wrap on. See [Example 6: Page Wrap](reporter-wrap.html) 
in the documentation for an illustration of how to wrap pages.

You can also turn the automatic
page wrapping off completely with `report_options(page_wrap = FALSE)` or `create_table(page_wrap = FALSE)`. See [Example 18: Report Options](reporter-report_options.html) for more
information.
...
If you're inserting 
code into your report with `report_options(allow_code = TRUE)`, please also 
double the curly braces if needed. See [Example 17: Code Insertion](reporter-codeinsert.html)
...
**Q:** I have some very long words in my data.  I want **reporter** to break them,
and wrap to the next line.  Is there a way to do that? 

**A:** No.  The **reporter** package will not break words.
What you should do is pre-process the column with the long words, 
and break it yourself.  You can use the `nchar()` function to check the character
length of the words.  Then insert a new line or blank space at the point where
you want to break. Once the words are broken where you want, send the data
into **reporter**.
...
**Q:** I'm trying to insert some RTF codes into my report, but they always
come out visible on the output.  Is there a way to insert them directly
into the RTF, without **reporter** interfering?

**A:** Yes.  **reporter** normally prevents such code insertion. But you
can allow it by setting `report_options(allow_code = TRUE)`.  The "allow_code"
option will work for RTF and HTML.  See the vignette 
[Example 17: Code Insertion](reporter-codeinsert.html) for an example.
...
**Q:** I'm trying to create some reports with **reporter** on a Mac, but
they seem to be consistently messed up.  RTF is really messed up and DOCX
doesn't come out right either. How can I fix it?

**A:** The **reporter** package was developed on Windows, and tested on Linux.
Those two platforms are supported best.  On a Mac,
TXT and PDF output types should be reliable. As for RTF and DOCX, they will work
or not depending on which editor you are using. RTF
and DOCX are best interpreted on Microsoft Word for Mac. For HTML, the content
produced will be correct, but usually the page breaking is off.  The reason
is because Safari has different defaults and different interpretations of some
HTML tags and styles. You can improve the HTML output by viewing on Chrome for Mac.
