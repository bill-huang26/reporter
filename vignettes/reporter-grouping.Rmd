...
In AE tables and summary statistics tables, there are groups for each section,
and sometimes we want to avoid splitting the same group across multiple pages. With
group cohesion, the **reporter** package can keep the group on in the same page 
as much as possible by setting `group_cohesion = TRUE` in `define()`.
...
fp <- file.path(tempdir(), "example16c.pdf")
...
# fp <- file.path("C:/Users/HuangC3/OneDrive - Parexel/Documents/reporter_bill/tests/testthat/user/example16c.pdf")
