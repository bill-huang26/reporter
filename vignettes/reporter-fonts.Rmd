...
three additional fonts: Arial, 
Times New Roman, and SimSun.  The package also supports the ability 
...
```
<img src="../man/images/fonts.png" alt="Font example image"/>

### Chinese Language Support

The "SimSun" font supports reporting in the Chinese language.  The font can be 
specified as the other English language fonts.  Here is an example of a report
using "SimSun" font:
```
    # Read in prepared data
    df <- read.table(header = TRUE, text = '
    ...
    tbl <- create_table(df, borders = "all") %>%
      ...
      "用来测试空白行的宽度是否需要缩减。切割文字的函數需 修改，分割不需要用空白："), 
              borders = "all") %>%
...
<img src="../man/images/fonts2.png" alt="SimSun example report"/>
