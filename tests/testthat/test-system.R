...
test_that("text104: Page wrap can be turned off as expected.", {
  
  if (dev) {
    fp <- file.path(base_path, "output/test104.txt")
    ...
    rpt <- create_report(fp, font = "fixed", output_type = "txt",
                         orientation = "portrait") %>%
...
test_that("text105: auto_page can be turned off as expected.", {
  
  if (dev == TRUE) {
    fp <- file.path(base_path, "output/test105.txt")
...
test_that("test106: line_count can be set in report_options as expected.", {
  
  if (dev == TRUE) {
    fp <- file.path(base_path, "output/test106.txt")
...
test_that("test107: Wrapping of footnote, header, title, footer, and page_by work as expected.", {
  
  if (dev) {
    fp <- file.path(base_path, "output/test107.txt")
...
test_that("test108: Footnote and title can be wrapped with multiple columns.", {
  
  if (dev) {
    fp <- file.path(base_path, "output/test108.txt")
...
test_that("test109: Title header can be wrapped.", {
  
  if (dev) {
    fp <- file.path(base_path, "output/test109.txt")
...
test_that("test113b: Percentage column widths work with 100% as expected.", {
  
  if (dev == TRUE) {
    fp <- file.path(base_path, "output/test113b.txt")
