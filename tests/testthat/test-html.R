...
test_that("html-59: Center page header works as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test59.html")
    ...
    rpt <- create_report(fp, output_type = "html", font = fnt,
      ...
      add_content(tbl) %>%
      footnotes("My footnote 1", "My footnote 2", borders = "none") %>%
      page_header(left = "l", center = "center", right = "right",
                  width = c(1, 4, 4)) %>%
...
test_that("html-60: Images in page header/footer work as expected.", {
  
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test60.html")
...
test_that("html-61: Spanned images in page header/footer work as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test61.html")
...
test_that("html-62: Return error when header_image/footer_image is used without page_header/page_footer.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test62.html")
...
test_that("html-63: group_cohesion puts the group in same page as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test63.html")
...
test_that("html-64: group_cohesion with min page proportion works as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test64.html")
    ...
    tbl <- create_table(dat, borders = "outside") %>%
      define(test_group, group_cohesion = 0.65,
...
test_that("html-65: Multiple group_cohesion work as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test65.html")
    ...
    dat$test_string <- c(
      ...
      c(rep("Flower C1", 17)),
      c(rep("Flower C2", 5)),
      c(rep("Flower D", 19), rep("Flower D\nSubgroup D1", 2)), # 20 lines + 1
      rep("Flower D2", 7),
      c(rep("Flower E1", 2)),
      c(rep("Flower E2", 19))
    )
...
test_that("html-66: Multiple group_cohesion with different cohesion strengths work as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test66.html")
    ...
    # C2 would not be moved to next page
    # E2 would be moved to next page because previous lines are 31, which meets 34*0.9
    expect_equal(file.exists(fp), TRUE)
  } else {
    expect_equal(TRUE, TRUE)
  }
...
test_that("html-67: break_label works as expected.", {
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test67.html")
...
test_that("html-68: Page numbers work every in title, footnote, header, and footer.", {
  ...
  if (dev) {
    fp <- file.path(base_path, "html/test68.html")
...
test_that("html-69: Page wrap can be turned off as expected.", {
  
  if (dev) {
    fp <- file.path(base_path, "html/test69.html")
...
test_that("html-70: Line break can be turned off by report_options as expected.", {
  
  if (dev) {
    fp <- file.path(base_path, "html/test70.html")
    ...
    for (r in html) {
      matches <- gregexpr("<br>", r, fixed = TRUE)
...
test_that("html-71: auto_page can be turned off as expected.", {
  
  if (dev == TRUE) {
    fp <- file.path(base_path, "html/test71.html")
